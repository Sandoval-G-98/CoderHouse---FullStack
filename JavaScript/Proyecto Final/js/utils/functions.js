    ...
    const invest = new Investment(
        ...
        if (totalAmount != -1){
            ...
            if(isWishedAmountReached(invest.wishedAmount, invest.amountPerYear) == true){
                ...
                showMessageAmountWished(`Felicidades! Su monto deseado fue alcanzado el año ${yearWishedAmount}`, true)
            } else {
                showMessageAmountWished("No llegó a su monto deseado :( , es momento de invertir más", false)
            }
...
function showAmounts(amountPerYear, yearsTocalculate) {
    let listAmountsPerYear = document.getElementById("listAmountPerYear")
...
function showMessageAmountWished(message, amountIsReached) {
